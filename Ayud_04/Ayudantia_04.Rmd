...
class(mapa_comunas$codigo_comuna)
class(pobreza$Cod.Com)
```
...
names(pobreza)[names(pobreza) == "Porc.Pobr"] <- "PORCPOBR" ### modifico el nombre.
pobreza[, Cod.Com := ifelse(Cod.Com < 10000 , paste0("0",pobreza$Cod.Com), pobreza$Cod.Com)]## cambio str a int, para hacer un merge.
pobreza$PORCPOBR <- as.numeric(pobreza$PORCPOBR)### Cambio a entero.
```
...
mapa <- merge(mapa,pobreza, by.x = "codigo_comuna", by.y = "Cod.Com")### merge entre ambas.
mapa$codigo_comuna <- as.factor(mapa$codigo_comuna)
mapa$codigo_region <- as.numeric(mapa$codigo_region)### filtro por numero para la región, ya que voy a querer ver distintas regiones.
```
...
```{r ggplot 1}
REGIONES1 <- unique(pobreza$Region)### vector que saque los nombres de todas las regiones.

GRAFICO_1_REG <-
  ggplot(data = mapa[codigo_region == params$reg,]) +
  geom_sf(aes(geometry = geometry, fill = PORCPOBR)) +### el fill va a ser por el nivel de pobreza, el cual se cambió a nivel numérico.
  xlab('Longitud') +
  ylab('Latitud') +
  labs(title = REGIONES1[params$reg], subtitle = 'Tasa de pobreza relativa comunal') +
  scale_fill_continuous(trans = 'reverse', labels = scales::percent_format(suffix = "%"))### mi error en el control, tenía que poner la escala al revés, para que mientras más oscuro, "sea una mayor cantidad de personas" o en este caso un mayor nivel de pobreza en la tercera región.
...
```{r merge 2}
library(data.table)
censo <- data.table(censo_2017_comunas) ### creo una data table de censo.
censo <- censo[,.(`Población` = sum(poblacion)), by = .(codigo_comuna)] ## sumo la población por codigo comuna.
mapa <- merge(mapa,censo, by = "codigo_comuna")### hago el merge entre mapa y censo.
mapa[, Pobres := `Población`*PORCPOBR] ### agrego una columna de la cantidad de personas pobres por comuna.
```
...
hospitales$COD_REGION[grepl("Arica",hospitales$NOM_REGION)] <- 15 ### quiero que le cambies el codigo de region a 15
...
```{r filtrando}
# Filtramos
rbd <- rbd[rbd$COD_REGION == params$reg & rbd$DEPENDENCI == 'municipal',] ### quiero que la dependencia sea municipal.
